...
export function generateBridge(info: UpstreamInfo): string {
  ...
  const toolNames = info.tools.map((tool) => tool.name).join(", ");
  const toolsJson = JSON.stringify(info.tools, null, 2);
...
const TOOLS = ${indent(toolsJson, 0)} as const;
...
export class GeneratedBridge {
  ...
  getToolDefinitions(): Array<(typeof TOOLS)[number]> {
    return [...TOOLS];
  }
