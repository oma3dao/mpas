...
export function validateSessionRequest(value: unknown): CoordinationSessionRequest {
  ...
  if (typeof body.did !== "string" || !body.did.startsWith("did:")) {
    throw new CoordinationStoreError(400, "INVALID_REQUEST", "CoordinationSessionRequest.did must be a DID.");
  }
...
export function validatePollRequest(value: unknown): CoordinationPollRequest {
  ...
  if (typeof body.did !== "string" || !body.did.startsWith("did:")) {
    throw new CoordinationStoreError(400, "INVALID_REQUEST", "CoordinationPollRequest.did must be a DID.");
  }
  if (body.cursor !== undefined && typeof body.cursor !== "string") {
    throw new CoordinationStoreError(400, "INVALID_REQUEST", "CoordinationPollRequest.cursor must be a string.");
  }
...
export function validateCoordinationActionRequest(value: unknown): CoordinationActionRequest {
  ...
  if (!isRecord(body.actionPackage) || body.actionPackage.type !== "ActionPackage") {
    throw new CoordinationStoreError(400, "INVALID_REQUEST", "CoordinationActionRequest.actionPackage is required.");
  }
  if (!isRecord(body.authorizationRequirements) || body.authorizationRequirements.type !== "AuthorizationRequirements") {
    throw new CoordinationStoreError(400, "INVALID_REQUEST", "CoordinationActionRequest.authorizationRequirements is required.");
  }
...
export function validateApprovalSubmission(value: unknown): CoordinationApprovalSubmission {
  ...
  if (!isRecord(body.actionEnvelopeHash) || typeof body.actionEnvelopeHash.value !== "string") {
    throw new CoordinationStoreError(400, "INVALID_REQUEST", "CoordinationApprovalSubmission.actionEnvelopeHash is required.");
  }
  if (!isRecord(body.approval) || body.approval.type !== "Approval") {
    throw new CoordinationStoreError(400, "INVALID_REQUEST", "CoordinationApprovalSubmission.approval is required.");
  }
  validateBodyIdempotency(body.idempotencyKey);
...
export function validateCancelRequest(value: unknown): CoordinationActionCancelRequest {
  ...
  if (!isRecord(body.actionId) || typeof body.actionId.value !== "string" || typeof body.proposerDid !== "string") {
    throw new CoordinationStoreError(400, "INVALID_REQUEST", "Cancellation actionId and proposerDid are required.");
  }
  validateBodyIdempotency(body.idempotencyKey);
...
export function effectiveIdempotencyKey(bodyValue: unknown, headerValue: unknown): string | undefined {
  try {
    ...
    throw new CoordinationStoreError(400, "idempotency_mismatch", error instanceof Error ? error.message : String(error));
  }
}
...
function invalidRequest(error: unknown): CoordinationStoreError {
  return new CoordinationStoreError(400, "INVALID_REQUEST", error instanceof Error ? error.message : String(error));
}

function requireRecord(value: unknown): Record<string, unknown> {
  if (!isRecord(value)) throw new CoordinationStoreError(400, "INVALID_REQUEST", "Request body must be a JSON object.");
  return value;
}

function requireVersionType(body: Record<string, unknown>, type: string): void {
  if (body.version !== "1" || body.type !== type) {
    throw new CoordinationStoreError(400, "INVALID_REQUEST", `Expected version 1 ${type}.`);
  }
}
