...
export interface VerifierCoordinationClient {
  pollWork(options?: { cursor?: string }): Promise<CoordinationPollResponse>;
  submitActionResponseDelivery(
...
export interface VerifierCoordinationState {
  version: "1";
...
export interface VerifierCoordinationStateStore {
  load(identity: { coordinationUrl: string; verifierDid: Did }): Promise<VerifierCoordinationState>;
  save(state: VerifierCoordinationState): Promise<void>;
}

export interface VerifierCoordinationWorkerEvent {
...
export interface VerifierCoordinationWorkerOptions {
  coordinationUrl: string;
  verifierDid: Did;
  ...
  onEvent?: (event: VerifierCoordinationWorkerEvent) => void;
  now?: () => number;
}

export type VerifierCoordinationWorkerStatus =
  | "idle"
  | "connecting"
  | "connected"
  | "retrying"
...
/**
 * Outbound Verifier worker for Coordination Service delivery.
 *
 ...
 */
export class VerifierCoordinationWorker {
  private readonly coordinationUrl: string;
  private readonly verifierDid: Did;
  private readonly client: VerifierCoordinationClient;
  private readonly stateStore: VerifierCoordinationStateStore;
  private readonly processAction: VerifierCoordinationWorkerOptions["processAction"];
  private readonly fallbackPollIntervalMs: number;
  private readonly reconnectInitialMs: number;
  private readonly reconnectMaxMs: number;
  private readonly onEvent?: VerifierCoordinationWorkerOptions["onEvent"];
  private readonly now: () => number;
  private readonly abortController = new AbortController();
  private state: VerifierCoordinationState;
  private status: VerifierCoordinationWorkerStatus = "idle";
  private socket?: CoordinationWebSocket;
  private runPromise?: Promise<void>;
  private pollChain: Promise<void> = Promise.resolve();
  private fallbackTimer?: ReturnType<typeof setInterval>;
  private fatalError?: unknown;

  private constructor(options: VerifierCoordinationWorkerOptions, state: VerifierCoordinationState) {
    this.coordinationUrl = normalizeUrl(options.coordinationUrl);
  ...
  static async create(options: VerifierCoordinationWorkerOptions): Promise<VerifierCoordinationWorker> {
    const identity = {
      coordinationUrl: normalizeUrl(options.coordinationUrl),
      verifierDid: options.verifierDid,
    };
    const state = await options.stateStore.load(identity);
    return new VerifierCoordinationWorker(options, state);
  }

  getStatus(): VerifierCoordinationWorkerStatus {
    return this.status;
  }
  ...
  private async runConnectionLoop(): Promise<void> {
    ...
    while (!this.abortController.signal.aborted && !this.fatalError) {
      ...
      try {
        ...
        retryError = error;
        this.socket?.close(1011, "Verifier coordination retry");
        this.socket = undefined;
        if (this.abortController.signal.aborted) break;
  ...
  private async drainAvailableWork(): Promise<void> {
    ...
    while (!this.abortController.signal.aborted) {
      const page = await this.client.pollWork(cursor === undefined ? {} : { cursor });
      ...
      if (deliveries.length > 0) {
        if (!page.nextCursor || page.nextCursor === previousCursor) {
          throw new CoordinationResponseError(
            "Coordination Service returned deliveries without an advancing cursor.",
          );
        }
  ...
  private async processDelivery(delivery: DeliveryEnvelope): Promise<void> {
    ...
    try {
      ...
      throw new CoordinationResponseError("Coordination Service returned an invalid Action request envelope.", {
        cause: error,
      });
    }
    if (!envelope.recipients.includes(this.verifierDid)) {
      throw new CoordinationResponseError("Polled Action request does not address this Verifier DID.");
    }
    ...
    if (!cached) {
      ...
      if (response.verifier?.did !== this.verifierDid) {
        throw new CoordinationResponseError("Verifier response DID does not match the configured Verifier DID.");
      }
      if (
        response.actionEnvelopeHash?.alg !== expectedHash.alg ||
        response.actionEnvelopeHash.value !== expectedHash.value
      ) {
        throw new CoordinationResponseError("Verifier response does not match the delivered Action Envelope hash.");
      }
    ...
    if (!cached.delivered) {
      const result = await this.client.submitActionResponseDelivery(cached.envelope);
      if (result.accepted !== true) {
        throw new CoordinationResponseError("Coordination Service did not accept the Verifier response delivery.");
      }
  ...
  private enterFatal(error: unknown): void {
    ...
    this.socket?.close(1008, "Verifier coordination failure");
    this.emit({
      level: "error",
      event: "fatal_error",
  ...
  private async saveState(): Promise<void> {
    try {
      ...
      throw new CoordinationResponseError("Unable to persist Verifier coordination state.", { cause: error });
    }
  }

  private emit(event: Omit<VerifierCoordinationWorkerEvent, "coordinationUrl" | "verifierDid">): void {
    this.onEvent?.({
      ...event,
      coordinationUrl: this.coordinationUrl,
      verifierDid: this.verifierDid,
    });
  }
}

/** Durable JSON state for a single Coordination Service origin and Verifier DID. */
export class FileVerifierCoordinationStateStore implements VerifierCoordinationStateStore {
  ...
  async load(identity: { coordinationUrl: string; verifierDid: Did }): Promise<VerifierCoordinationState> {
    ...
    try {
      ...
      throw new Error(`Unable to read Verifier coordination state: ${this.path}`, { cause: error });
    }
...
function newState(identity: { coordinationUrl: string; verifierDid: Did }): VerifierCoordinationState {
  return {
    version: "1",
    type: "MpasVerifierCoordinationState",
    coordinationUrl: normalizeUrl(identity.coordinationUrl),
...
function parseState(
  value: unknown,
  identity: { coordinationUrl: string; verifierDid: Did },
  path: string,
): VerifierCoordinationState {
  if (!isRecord(value) || value.version !== "1" || value.type !== "MpasVerifierCoordinationState") {
    throw new Error(`Verifier coordination state is invalid: ${path}`);
  }
  const coordinationUrl = normalizeUrl(identity.coordinationUrl);
  if (value.coordinationUrl !== coordinationUrl || value.verifierDid !== identity.verifierDid) {
    throw new Error(
      `Verifier coordination state identity does not match ${coordinationUrl} and ${identity.verifierDid}: ${path}`,
    );
  }
  if (value.cursor !== undefined && typeof value.cursor !== "string") {
    throw new Error(`Verifier coordination cursor is invalid: ${path}`);
  }
  if (!isRecord(value.responses)) {
    throw new Error(`Verifier coordination response cache is invalid: ${path}`);
  }
  const responses: VerifierCoordinationState["responses"] = {};
  for (const [fingerprint, entry] of Object.entries(value.responses)) {
    if (!fingerprint || !isRecord(entry) || typeof entry.delivered !== "boolean") {
      throw new Error(`Verifier coordination response cache entry is invalid: ${path}`);
    }
...
function responseRecipients(proposer: Did, response: ActionResponse): Did[] {
  const recipients = new Set<Did>([proposer]);
  const requirements = response.authorizationRequirements;
  if (requirements?.result === "additionalApprovalsRequired") {
    for (const threshold of requirements.approvalRequirements.anyOf ?? []) {
      for (const signer of threshold.eligibleSigners) recipients.add(signer);
    }
    for (const threshold of requirements.approvalRequirements.allOf ?? []) {
      for (const signer of threshold.eligibleSigners) recipients.add(signer);
    }
    for (const override of requirements.approvalRequirements.overrideSigners ?? []) {
      recipients.add(override.signer);
    }
  }
  return [...recipients];
}
...
function errorCode(error: unknown): string {
  ...
  if (error instanceof CoordinationUnavailableError) return error.code;
  if (error instanceof CoordinationResponseError) return error.code;
  return "VERIFIER_COORDINATION_ERROR";
}

function safeMessage(error: unknown): string {
  if (error instanceof Error) return error.message;
  return "Verifier coordination failed.";
}

function waitForSocketEnd(socket: CoordinationWebSocket, signal: AbortSignal): Promise<void> {
