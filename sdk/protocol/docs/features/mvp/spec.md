# MPAS Protocol SDK — Specification

**Status:** Draft  
**Package:** `@oma3/mpas`  
**Directory:** `sdk/protocol/`  
**Depends on:** MPAS Core Specification v0.2, MPAS Application Plugin Profile v0.2, MPAS Policy Profile v0.2, MPAS HTTP Profile v0.2, MCP Protocol (JSON-RPC over stdio/HTTP)

---

## 1. Overview

The MPAS Protocol SDK (`@oma3/mpas`) is the canonical TypeScript implementation of MPAS protocol primitives. It provides everything needed to build MPAS integrations — whether you're building a Proposer agent, a Verifier/Adapter, a Signer server, or a Coordination Service.

The package provides:

- **Protocol Types** — Complete TypeScript type definitions for all MPAS artifacts (ActionPackage, ActionEnvelope, Approval, AuthorizationRequirements, ExecutionReceipt, etc.)
- **Verification** — Parse, validate, and cryptographically verify Action Packages, Approval Bundles, and payload bindings.
- **Policy Engine** — Evaluate JSON policy configurations against Action Packages (conditions, thresholds, allOf/anyOf composition).
- **Plugin Loader** — Load and validate MPAS Application Plugin JSON files, validate Execution Payloads against plugin schemas.
- **Receipt Builder** — Construct and sign Execution Receipts (JWS).
- **Auth Requirements Builder** — Construct AuthorizationRequirements responses from unsatisfied policy rules.
- **Proposer Primitives** — ActionPackageBuilder, ApprovalBuilder, PluginToolGenerator for constructing MPAS artifacts from tool calls.
- **Protocol Clients** — AdapterClient (HTTP client for Credential Adapters) and CoordinationClient (HTTP client for Coordination Services).
- **Key Management** — Ed25519 key loading, `did:key` derivation, JWS signing/verification.
- **DID Utilities** — `did:key` derivation, key generation, kid computation.
- **Trace/Observability** — Structured JSONL trace logging for protocol events.
- **MCP Bridge** — ProposerBridge class that wraps tool calls in MPAS artifacts and routes through a Credential Adapter.

**Organizing principle:** If it's defined by the MPAS specs (core, plugin profile, policy profile, HTTP profile), it belongs in this SDK. Implementation choices (what HTTP framework to use, how to store credentials, how to dispatch to target applications) belong in consumer code.

The **MPAS Signer Server** — a standalone MCP server that enables agents to act as Signers — lives separately in `examples/demo/src/signer-server/`. It is a consumer of this SDK's primitives.

---

## 2. Goals

- Provide a single, canonical TypeScript implementation of all MPAS protocol primitives.
- Enable developers to build any MPAS role (Proposer, Verifier, Signer, Coordination Service) using shared, tested building blocks.
...
- Support the Verifier role: parse, validate, and cryptographically verify incoming Action Packages; evaluate policy; build receipts and auth-requirements responses.
- Support the Signer role: provide protocol primitives (`CoordinationClient`, `ApprovalBuilder`, `KeyManager`) that signer implementations use.
- Generate MCP tool definitions from MPAS Application Plugin JSON (auto-registration of tools from plugin schemas).
- Remain neutral to agent framework (works with any MCP client or non-MCP integration).
- Remain neutral to Credential Adapter implementation (works with any adapter that speaks the `/mpas/v1/action` interface).
- Remain neutral to transport and runtime (no HTTP framework dependency in core primitives).

---

## 3. Non-Goals

- Implementing a full Credential Adapter HTTP server (that's `examples/demo`).
- Implementing a full Coordination Service (that's `examples/demo`).
- Defining MPAS protocol semantics (that's the MPAS specification documents).
- Managing application credentials or executing actions against target applications.
- Requiring agents to understand MPAS protocol details.
- Choosing an HTTP framework, database, or deployment strategy.
...
### 4.2 SDK vs. Signer Server

The SDK provides the **ProposerBridge** — a library class for building MCP servers that wrap application tool calls in MPAS artifacts. The **Signer Server** (formerly MaintainerBridge) is a separate, standalone MCP server that lives in `examples/demo/src/signer-server/`. It is a *consumer* of the SDK's protocol primitives, not a library component.

| Component          | Location                              | What it does                                                                                            | Example tools exposed                                                        |
| :----------------- | :------------------------------------ | :------------------------------------------------------------------------------------------------------ | :--------------------------------------------------------------------------- |
| **ProposerBridge** | SDK (`@oma3/mpas`)                    | Wraps tool calls in Action Packages, submits to adapter, polls coordination for its own actions' state  | `merge_pull_request`, `delete_branch` (same as the real server)|
| **Signer Server**  | `examples/demo/src/signer-server/`    | Queries coordination on demand for pending approval requests for the agent's DID, lets agents approve   | `mpas_list_pending`, `mpas_review_action`, `mpas_approve`, `mpas_reject`     |
...
- One Signer Server instance (handles approvals across all applications)
...
The Signer Server is the single point of contact for all approval requests. When the agent calls `mpas_list_pending`, the server queries the coordination service with the agent's DID and returns all pending approval requests regardless of which application they target.

---

## 5. Library vs. Examples

### 5.1 Library (`@oma3/mpas`)

The published npm package (`sdk/protocol/`). Provides:

**Protocol Verification (Verifier-side):**
- **`parseActionPackage`** — Structurally validate an incoming JSON blob as an ActionPackage.
- **`validateActionEnvelope`** — Validate all Action Envelope fields, DID formats, timestamps, expiry.
- **`verifyPayloadBinding`** — Verify the Execution Payload hash matches the Action Envelope.
- **`verifyApprovalSignature`** — Verify a single Approval's JWS signature against a public key.
- **`verifyApprovalBundle`** — Verify all approvals in a bundle against trusted signers.
- **`verifyActionPackage`** — Full end-to-end verification pipeline (envelope → binding → bundle).
- **`computeJsonHash`** — SHA-256 hash of JCS-canonicalized JSON.
- **`isEnvelopeExpired`** / **`exceedsMaxEnvelopeValidity`** — Expiry checks.
...
- **`validatePayloadAgainstPlugin`** — Validate an ExecutionPayload against a plugin's operation schemas.

**Receipt & Auth Requirements:**
- **`buildAndSignReceipt`** — Construct and sign an ExecutionReceipt (JWS).
- **`buildAuthorizationRequirements`** — Construct AuthorizationRequirements from unsatisfied policy rules.

**Proposer Primitives:**
- **`ActionPackageBuilder`** — Construct Action Packages from tool calls (payload → envelope → approval → package).
...
**Protocol Clients:**
- **`AdapterClient`** — HTTP client for submitting Action Packages to a Credential Adapter.
- **`CoordinationClient`** — HTTP client for interacting with a Coordination Service.

**Key Management & DID:**
- **`KeyManager`** — Load Ed25519 keys, derive `did:key`, sign JWS.
- **`deriveDidKey`** / **`generateEd25519Key`** / **`didKeyToKid`** — DID key utilities.

**Trace/Observability:**
- **`TraceWriter`** / **`TraceLogger`** — Structured JSONL trace logging.

**MCP Bridge:**
- **`ProposerBridge`** — Full MCP server that wraps tool calls in MPAS artifacts.
...
These are usable without instantiating a `ProposerBridge`. A developer building a custom workflow (e.g., a CLI proposer, a batch approval script, a signer server, or a non-MCP integration) can import and compose these directly.
...
### 5.4 Signer Server (`examples/demo/src/signer-server/`)

The MPAS Signer Server (formerly MaintainerBridge) is a standalone runnable MCP server that lives in the `examples/demo` package. It is a consumer of the SDK's protocol primitives — not an SDK library component. It polls a Coordination Service for pending approval requests and exposes review/approve/reject tools.
...
For each operation in the plugin's `operations` object, the bridge registers an MCP tool with:
- `name` = the operation key
...
The MPAS Signer MCP Server is a standalone MCP server — one per agent — that enables an agent to participate in MPAS approval flows as a Signer across all applications. It lives in `examples/demo/src/signer-server/` (not in the SDK library). When the agent calls `mpas_list_pending`, the server queries the Coordination Service with the agent's DID and returns pending approval requests. It exposes tools that allow the agent to:
...
```json
{
  "mcpServers": {
    ...
    "mpas-signer": {
      "command": "npx",
      "args": ["tsx", "/path/to/examples/demo/src/signer-server/index.ts", "--config", "./bridge-configs/signer.json"]
    }
  }
}
```
...
### 9.2 Signer Server (separate package)

The Signer Server (`SignerServer` class) has been extracted from the SDK and lives in `examples/demo/src/signer-server/`. It is a standalone MCP server that consumes the SDK's protocol primitives (`CoordinationClient`, `ApprovalBuilder`, `KeyManager`). See `examples/demo/src/signer-server/index.ts` for the full implementation.
...
class CoordinationClient {
  ...
  // Poll the coordination service (used by ProposerBridge and the Signer Server)
...
1. **Auto-signer mode:** Should the Signer Server support a mode where it automatically approves actions matching certain criteria (e.g., "approve all create_issue from did:key:z6Mk...")? This enables deterministic agent signers without LLM judgment. Needs careful safety guardrails.
