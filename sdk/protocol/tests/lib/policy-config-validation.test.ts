...
describe("validatePolicyConfig", () => {
  ...
  it("rejects invalid match source and op", () => {
    ...
    policy.policies = {
      create_issue: [
        {
          match: {
            conditions: [{ source: "payload", path: "/x", op: "eq", value: 1 } as never],
          },
          requirements: { type: "proposerOnly" },
        },
      ],
    ...
    policy.policies = {
      create_issue: [
        {
          match: {
            conditions: [{ source: "executionPayload", path: "/x", op: "regex" as never, value: "a" }],
          },
          requirements: { type: "proposerOnly" },
        },
      ],
  ...
  it("rejects unsupported fields on proposerOnly", () => {
    const policy = validPolicy();
    policy.defaultRequirement = { type: "proposerOnly", decision: "approve" } as never;
    expect(validatePolicyConfig(policy)).toMatchObject({
      ok: false,
  ...
  it("rejects non-object defaultRequirement values", () => {
    ...
    expect(validatePolicyConfig({ ...base, defaultRequirement: null } as never)).toMatchObject({
      ok: false,
      message: expect.stringContaining("approval requirement object"),
    });
    expect(validatePolicyConfig({ ...base, defaultRequirement: "x" } as never)).toMatchObject({
      ok: false,
      message: expect.stringContaining("approval requirement object"),
    });
  });
