...
describe("validateActionEnvelope (sdk)", () => {
  ...
  it("rejects structural envelope field violations", async () => {
    ...
    expect(
      validateActionEnvelope({ ...base, target: { applicationDid: "web:example" as never } }),
    ).toMatchObject({
      ok: false,
    ...
    expect(
      validateActionEnvelope({
        ...base,
        executionPayloadHash: { ...base.executionPayloadHash, alg: "sha-512" as never },
      }),
    ).toMatchObject({
      ok: false,
...
describe("verifyPayloadBinding (sdk)", () => {
  ...
  it("returns false when the envelope hash algorithm is not sha-256", async () => {
    ...
    const envelope = {
      ...actionPackage.actionEnvelope,
      executionPayloadHash: { ...actionPackage.actionEnvelope.executionPayloadHash, alg: "sha-512" as never },
    };
...
describe("resolveTrustedSignerJwk (sdk)", () => {
  it("prefers did:jwk derivation over an explicit publicJwk", async () => {
    ...
    const resolved = resolveTrustedSignerJwk({ did: proposer.did, publicJwk: proposer.publicJwk as never });
