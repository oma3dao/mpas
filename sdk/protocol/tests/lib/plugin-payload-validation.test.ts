...
describe("validatePayloadAgainstPlugin (sdk)", () => {
  ...
  it("reuses the compiled schema cache for the same operation descriptor", async () => {
    const plugin = await githubPlugin();
    const payload = {
      name: "merge_pull_request_mirror",
      arguments: {
    ...
    const first = validatePayloadAgainstPlugin(payload, plugin);
    const second = validatePayloadAgainstPlugin(payload, plugin);
