...
export interface ActionResponse {
  ...
  /**
   * INFORMATIVE execution-profile-native response content (for mcp.toolsCall, the
   * target server's MCP tools/call result object, verbatim). Not hash-bound, not
   * covered by the receipt signature, not an attestation of output. Present for
   * `executed` and for `failed` when the target returned a tool-level failure
   * (isError: true); absent for protocol-level `failed`, `indeterminate`, and all
   * pure-MPAS outcomes.
   */
