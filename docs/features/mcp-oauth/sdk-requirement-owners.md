...
| RFC 8414/OIDC authorization-server discovery | SDK + CA validation/fetch policy | SDK discovers the path-bearing fixture issuer but v1.30.0 accepts metadata whose declared `issuer` does not match it. The CA provider now rejects mismatches in `saveDiscoveryState`, before authorization starts or discovery is persisted. Bounded fetches and exact probe-order cases remain. |
| PKCE S256 and authorization URL construction | SDK | The test verifies the saved verifier, derived S256 challenge, redirect URI, state, scope, and exact resource indicator. The SDK rejects authorization-server metadata that advertises only `plain`. The CA owns authorization-session entropy, expiry, and single use. |
...
| HTTPS, redirect, response-size, timeout, and header-forwarding limits | CA fetch/transport policy | The wrapper requires HTTPS (with explicit loopback-only fixture opt-in), rejects redirects, applies a bounded timeout, limits buffered JSON responses, and permits Bearer authorization only on the configured exact MCP resource URL. |
